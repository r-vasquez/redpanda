...
// stopTimeout is the maximum time to wait for the spinner to stop gracefully.
...
func NewSpinner(ctx context.Context, message string, opts ...SpinnerOption) *Spinner {
	...
	s.program = tea.NewProgram(model,
		...
		// Disable input to prevent raw mode (e.g Ctrl+C generates SIGINT) and
		// let the signal propagate to terminate the entire command, not just
		// the spinner.
		tea.WithInput(nil),
...
func (s *Spinner) stopWith(finalMsg string) {
	...
	if s.program != nil {
		...
		// Wait for the program to finish with a timeout to prevent blocking forever.
		select {
		case <-s.done:
